...
export const useAuthStore = create<AuthState>((set, get) => ({
  ...
  setUser: (user) => {
    if (user) {
      ...
      Cookies.set("parkshare_role", user.role, { expires: 7, secure, sameSite: "lax" });
  ...
  fetchCurrentUser: async () => {
    ...
    try {
      ...
      const secure = isSecureContext();
      Cookies.set("parkshare_token", session.access_token, { expires: 7, secure, sameSite: "lax" });
      if (typeof window !== "undefined") {
        localStorage.setItem("parkshare_token", session.access_token);
      }
